...
const string strCur = "camera1.txt", strLost = "lost1.txt";
ofstream fCur, fL;
...
int main(int argc, char **argv)
{
    ...
    ros::start();
    ros::NodeHandle nh;
    string orbSetiingsFile = "/home/rabot/catkin_ws/src/ros_orbslam/data/camera1.yaml";
    ...
    message_filters::Subscriber <sensor_msgs::Image> rgb_sub(nh, "/camera1/rgb/image_raw", 1);
    message_filters::Subscriber <sensor_msgs::Image> depth_sub(nh, "/camera1/depth_registered/image_raw", 1);
...
//        vo_broadcaster->sendTransform(tf::StampedTransform(vo_trans, stamp, "odom", "vo"));
        ...
        try{
          listener->lookupTransform("/base_link", "/camera1_link", stamp, tf_stamp);
        }
