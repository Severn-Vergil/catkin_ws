...
const string strCur = "camera2.txt", strLost = "lost2.txt";
ofstream fCur, fL;
...
int main(int argc, char **argv)
{
    ...
    ros::start();
    ros::NodeHandle nh;
    string orbSetiingsFile = "/home/rabot/catkin_ws/src/ros_orbslam/data/camera2.yaml";
    string orbVocFile = "/home/rabot/ORB_SLAM2/Vocabulary/ORBvoc.bin";
    stamp = ros::Time::now();
    vo_broadcaster = new tf::TransformBroadcaster();
    vo_pub = nh.advertise<nav_msgs::Odometry>("vo2", 1000);
    vor_pub = nh.advertise<nav_msgs::Odometry>("vo2_registered", 1000);
    ...
    message_filters::Subscriber <sensor_msgs::Image> rgb_sub(nh, "/camera2/rgb/image_raw", 1);
    message_filters::Subscriber <sensor_msgs::Image> depth_sub(nh, "/camera2/depth_registered/image_raw", 1);
...
//        vo_broadcaster->sendTransform(tf::StampedTransform(vo_trans, stamp, "odom", "vo"));
        ...
        try{
          listener->lookupTransform("/base_link", "/camera2_link", stamp, tf_stamp);
        }
        ...
        vor.header.frame_id = "vo2_registered";
