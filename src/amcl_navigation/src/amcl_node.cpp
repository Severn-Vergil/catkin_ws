...
/* Author: Brian Gerkey */
// 2016. 05. 13
// Add VO for interpolation of AMCL when covariance too large.
// 2016. 06. 23
// Not workable.
...
// assign odom_pose, x, y, yaw, with timestamp of laser_scan and base_frame_id_
bool
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  // the first message of laser scan.
  {
    ...
    try
    {
      ...
      ROS_INFO("laser_pose from scan [%d]: [%3.3f, %3.3f, %3.3f]", laser_scan->header.seq,
               laser_pose.getOrigin().getX(), laser_pose.getOrigin().getY(),
               tf::getYaw(laser_pose.getRotation())*180.0/M_PI);
    }
    catch(tf::TransformException& e)
    {
...
//      //TODO: Add VO interpolation of AMCL
//      // when var in x direction too huge, fuse VO with AMCL.
//      // particles are reset.
//      if(p.pose.covariance[0] > 0.25 || p.pose.covariance[7] > 0.25 || p.pose.covariance[35] > (M_PI/10.0) * (M_PI/10.0))
//      {
//          tf::StampedTransform tx_odom;
//          ros::Time t_last = last_published_pose.header.stamp, t_new = laser_scan->header.stamp;
//          try
//          {

//              // wait a little for the latest tf to become available
//              tf_->waitForTransform(base_frame_id_, t_last,
//                                    base_frame_id_, t_new,
//                                    odom_frame_id_, ros::Duration(0.5));
//              tf_->lookupTransform(base_frame_id_, t_last,
//                                   base_frame_id_, t_new,
//                                   odom_frame_id_, tx_odom);
//              // using VO for interpolation
////              tf_->waitForTransform("/vo", last_published_pose.header.stamp,
////                                    "/vo", t_new,
////                                    odom_frame_id_, ros::Duration(0.5));
////              tf_->lookupTransform("/vo", last_published_pose.header.stamp,
////                                   "/vo", t_new,
////                                   odom_frame_id_, tx_odom);
//          }
//          catch(tf::TransformException e)
...
//          tf::poseMsgToTF(last_published_pose.pose.pose, pose_old);
//          pose_new = pose_old * tx_odom;
//          //          tf::poseTFToMsg(pose_new, p);

//          // Fill in the header
//          p.header.frame_id = global_frame_id_;
//          // Copy in the pose
//          p.pose.pose.position.x = pose_new.getOrigin().getX();
//          p.pose.pose.position.y = pose_new.getOrigin().getY();
//          tf::quaternionTFToMsg(tf::createQuaternionFromYaw(getYaw(pose_new)),
//                                p.pose.pose.orientation);
////          // Reset covariance
////          for(int i=0; i<2; i++)
////          {
////              for(int j=0; j<2; j++)
////              {
////                  p.pose.covariance[6*i+j] = 0;
////              }
////          }
////          p.pose.covariance[0] = 0.4 * 0.4;                     // variance in x
////          p.pose.covariance[6*1+1] = 0.4 * 0.4;                 // variance in y
////          p.pose.covariance[6*5+5] = (M_PI/12.0) * (M_PI/12.0); // variance in yaw
...
//            for(int j=0; j<2; j++)
//            {
//              pf_init_pose_cov.m[i][j] = p.pose.covariance[6*i+j];
//            }
//          }
//          pf_init_pose_cov.m[2][2] = p.pose.covariance[6*5+5];

//          // update initial_pose_hyp_ to now by interpolation with odom.
...
AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  ...
  // let initialpose guess from other publisher works.
  // so comment codes below.
  ...
  // assume the mobile robot moves on a planar.
  ROS_INFO("tx_odom: x,y,theta: [%3.2f, %3.2f, %3.2f]",
           tx_odom.getOrigin().getX(), tx_odom.getOrigin().getY(), tx_odom.getRotation().getAngle());
