//grab raw depth image and convert it into real depth data.
...
void imageCallback(const sensor_msgs::ImageConstPtr& image) {
    const int u1 = 265, u2 = 275, v1 = 535, v2 = 545;
    int* depth_row = new int[v2-v1];
    for(int i = u1; i < u2; i++)
    {
        for(int j = v1; j < v2; j++)
        {
            depth_row[j-v1] = ReadDepthData(i, j, image);//u,v,d: Width = 960, Height = 540
            cout<<depth_row[j-v1]<<" ";
        }
        cout<<endl;
    }
...
int main(int argc, char **argv)
{
    ...
    image_transport::Subscriber sub = it.subscribe("/camera1/depth/image", 1, imageCallback);
    ros::spin();
    return 0;
}
